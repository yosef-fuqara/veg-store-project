...
/**
 * Find UTC epoch ms for a given calendar date + wall time in `timeZone`.
 * Scans a 50h window in 1-minute steps (fast enough; avoids DST math bugs).
 * @returns {number | null}
 */
...
/**
 * Next calendar date (y,m,d) in `timeZone` after the given zoned y,m,d.
 ...
 */
function evaluateWindow({ at, timeZone, openM, closeM }) {
  ...
  /* Overnight: open from openM until closeM next calendar morning (closeM < openM). */
  const within = nowM >= openM || nowM < closeM;
  if (within) return { within: true, nextOpenAtMs: null };
